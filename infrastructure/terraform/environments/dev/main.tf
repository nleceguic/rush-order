...
# ── Monitoring: LAW + APPI + Action Group ─────────────────────────────────────
module "monitoring" {
...
# ── Metric Alerts (defined here to avoid circular module dependency) ───────────
resource "azurerm_monitor_metric_alert" "availability" {
  name                = "rush-order-dev-alert-availability"
  resource_group_name = module.shared.resource_group_name
